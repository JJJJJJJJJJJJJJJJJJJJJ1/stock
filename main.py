...
# 1. 페이지 기본 설정
st.set_page_config(
    page_title="글로벌 IT 빅테크 주가 분석",
    page_icon="📈",
    layout="wide"
)

st.title("📈 글로벌 IT 기업 최근 1년 주가 분석 대시보드")
st.markdown("삼성전자, SK하이닉스, 구글, 마이크로소프트(MS), 애플의 최근 1년 주가 변동 및 누적 수익률을 비교합니다.")
...
def load_stock_data():
    ...
    for name, ticker in tickers.items():
        # 최근 1년 종가(Close) 데이터 수집
        df = yf.download(ticker, start=start_date, end=end_date)
        if not df.empty:
            # yfinance 최신 버전의 MultiIndex 또는 단일 컬럼 대응
            if isinstance(df.columns, pd.MultiIndex):
                combined_df[name] = df['Close'][ticker]
            else:
                combined_df[name] = df['Close']
                
    # 국가별 휴장일로 인한 결측치는 직전 데이터로 채움(Forward Fill)
...
# 4. 레이아웃 탭 구성
tab1, tab2, tab3 = st.tabs(["📊 누적 수익률 비교", "💵 개별 주가 추이", "📋 데이터 요약"])

# --- Tab 1: 누적 수익률 비교 (Plotly) ---
with tab1:
    st.subheader("기준일 대비 누적 수익률 (%) 비교")
    st.caption("※ 통화(KRW/USD)와 주가 규모가 다르므로, 최근 1년 첫 거래일을 0%로 맞추어 변동률을 비교합니다.")
    
    # 첫 거래일 대비 누적 수익률 계산
    normalized_data = (price_data / price_data.iloc[0] - 1) * 100
    
    # Plotly 인터랙티브 라인 차트 생성
    fig1 = go.Figure()
    for col in normalized_data.columns:
        fig1.add_trace(go.Scatter(
            ...
            mode='lines', 
            name=col,
            hovertemplate='%{x}<br>%{sidebar}: <b>%{y:.2f}%</b><extra></extra>'
        ))
...
with tab2:
    ...
    with col_chart:
        ...
        fig2.update_layout(
            title=f"{selected_stock} 1개년 주가 흐름 ({currency})",
            xaxis_title="날짜",
    ...
    with col_stat:
        st.markdown("<br><br>", unsafe_allow_html=True) # 간격 조정
        current_p = price_data[selected_stock].iloc[-1]
        start_p = price_data[selected_stock].iloc[0]
